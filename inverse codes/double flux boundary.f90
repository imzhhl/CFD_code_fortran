! Program for natural convection in enclosures
...
!    DIMENSION HFP1(NY)  ! DEFINE THE HEAT FLUX FUNCTION  
	INTEGER Lin,Lqu
	REAL HFP1, HFP2, HFP3, HFP4
	DIMENSION HFP1(NY), HFP2(NY), HFP3(NY), HFP4(NY)
	REAL Re, Gr, Pr
     ...
     ! OPEN(9,FILE=FILE2,STATUS='UNKNOWN')
      ...
      OPEN(9,FILE='sin-Ra1e4.dat',STATUS='UNKNOWN')
...
!				  XLe = 2.00;      DRT = 0.00000000010;      DRC = 0.00000000010
              ...
              OMIGU = 0.20;    OMIGV = 0.20;   OMIGPC = 0.90;    OMIGT = 0.8
!			  
...
!                          U(1,J) = +0.0
!                          V(1,J) = +0.0
		                   ! T(1,J) = T(2,J) ! + HDX(1)*HFP1(J)
						     ! ENDDO               !-(T(2,J) - T(1,J))/HDX(1) = 0

!RIGHT - the unknown Qu wall
                             DO J = 2,NJM
                                  U(NIM,J) = 0.0
                                V(NI, J) = 0.0
					      T(NI,J) = T(NIM,J) - HDX(NIM)*HFP1(J)
					         ENDDO
						  T(NI,1) = T(NI,2)
						 T(NI,NJ) = T(NI,NJM)
...
! firstly, above all, calculate TEH Qexact,
		 ...
		 ENDDO
! SAVE IT
         OPEN(13,FILE='Qexact2.dat',STATUS='UNKNOWN')
...
! firstly, above all, calculate TEH Qexact,
		 ...
		 ENDDO
! SAVE IT
         OPEN(14,FILE='Qexact1.dat',STATUS='UNKNOWN')
             DO J = 1,NJ				      !--------------------------Depending on the unknown function (1)
...
!!Record the emission rates along the heat sources - constant T type [OPEN] 
           ...
           !    ANuS = ANuS + DY(J)*( T(NI,J) - T(NIM,J))/HDX(NIM) 
                ANuS = ANuS + DY(J)* T(NI,J)
               ENDDO
            TotalNu = 1.00 / ANuS
           	   WRITE(*,*) 'Total heat transfer rate', TotalNu
...
!			 Imw = 81; Ime = 81
!              Jms =  2; Jmn = NJM
!         OPEN(3012,FILE='T-measured81.dat',STATUS='UNKNOWN')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(3012,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(3012)
!			 
...
!			 Imw = 5; Ime = 5
!              Jms =  2; Jmn = NJM
!         OPEN(302,FILE='T-measured5.dat')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(302,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(302)
!			 
...
!			 Imw = 77; Ime = 77
!              Jms =  2; Jmn = NJM
!         OPEN(3022,FILE='T-measured77.dat',STATUS='UNKNOWN')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(3022,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(3022)
!			 
!			 Imw = 9; Ime = 9
!              Jms =  2; Jmn = NJM
!         OPEN(303,FILE='T-measured9.dat')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(303,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(303)
!			 
...
!			 Imw = 73; Ime = 73
!              Jms =  2; Jmn = NJM
!         OPEN(3032,FILE='T-measured73.dat',STATUS='UNKNOWN')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(3032,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(3032)
!			 
!			 Imw = 13; Ime = 13
!              Jms =  2; Jmn = NJM
!         OPEN(304,FILE='T-measured13.dat')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(304,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(304)
!			 
...
!			 Imw = 69; Ime = 69
!              Jms =  2; Jmn = NJM
!         OPEN(3042,FILE='T-measured69.dat',STATUS='UNKNOWN')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(3042,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(3042)
!			 
!			 Imw = 17; Ime = 17
!              Jms =  2; Jmn = NJM
!         OPEN(305,FILE='T-measured17.dat')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(305,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(305)
!			 
...
!			 Imw = 65; Ime = 65
!              Jms =  2; Jmn = NJM
!         OPEN(3052,FILE='T-measured65.dat',STATUS='UNKNOWN')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(3052,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(3052)
!			 
!			 Imw = 21; Ime = 21
!              Jms =  2; Jmn = NJM
!         OPEN(306,FILE='T-measured21.dat')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(306,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(306)
!			 
...
!			 Imw = 61; Ime = 61
!              Jms =  2; Jmn = NJM
!         OPEN(3062,FILE='T-measured61.dat',STATUS='UNKNOWN')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(3062,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(3062)
!			 
!			 Imw = 25; Ime = 25
!              Jms =  2; Jmn = NJM
!         OPEN(307,FILE='T-measured25.dat')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(307,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(307)
!			 
...
!			 Imw = 57; Ime = 57
!              Jms =  2; Jmn = NJM
!         OPEN(3072,FILE='T-measured57.dat',STATUS='UNKNOWN')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(3072,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(3072)
!			 
!			 Imw = 29; Ime = 29
!              Jms =  2; Jmn = NJM
!         OPEN(308,FILE='T-measured29.dat')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(308,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(308)
!			 
...
!			 Imw = 53; Ime = 53
!              Jms =  2; Jmn = NJM
!         OPEN(3082,FILE='T-measured53.dat',STATUS='UNKNOWN')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(3082,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(3082)
!			 
!			 Imw = 33; Ime = 33
!              Jms =  2; Jmn = NJM
!         OPEN(309,FILE='T-measured33.dat')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(309,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(309)
!			 
...
!			 Imw = 49; Ime = 49
!              Jms =  2; Jmn = NJM
!         OPEN(3092,FILE='T-measured49.dat',STATUS='UNKNOWN')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(3092,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(3092)
!			 
...
!			  Imw = 37; Ime = 37
!              Jms =  2; Jmn = NJM
!         OPEN(310,FILE='T-measured37.dat')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(310,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(310)
!			 
...
!			 Imw = 45; Ime = 45
!              Jms =  2; Jmn = NJM
!         OPEN(3102,FILE='T-measured45.dat',STATUS='UNKNOWN')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(3102,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(3102)
!			 
...
!			 Imw = 41; Ime = 41
!              Jms =  2; Jmn = NJM
!         OPEN(311,FILE='T-measured41.dat')
!            
!			DO Im = Imw,Ime     
!            DO Jm = Jms-1,Jmn+1  
!              WRITE(311,115) YC(Jm), T(Im,Jm)
!            ENDDO
!  		    ENDDO
!             CLOSE(311)
!			 
...
!			 ! Imw = 41; Ime = 41
!              ! Jms =  2; Jmn = NJM
!         ! OPEN(3112,FILE='T-measured41.dat',STATUS='UNKNOWN')
!            
!			! DO Im = Imw,Ime     
!            ! DO Jm = Jms-1,Jmn+1  
!              ! WRITE(3112,115) YC(Jm), T(Im,Jm)
!            ! ENDDO
!  		    ! ENDDO
!             ! CLOSE(3112) 
!			 
...
!..Poisson Method for solving the	streamfunction
     ...
     SO(I,J) = (Ve-Vw)*DY(J)-(Un-Us)*DX(I)  
        ...
        ENDDO
             ...
             CALL SLORXY(PSI, 15000, ERROPSI, NIM, NJM, AE, AW, AN, AS, AP, SO, 0.95)  
...
!---Poisson Method for solving the	heatfunctions
             CALL SLORXY(HF, 15000, ERROHL, NIM, NJM, AE, AW, AN, AS, AP, SO, 0.95)  	!!!For higher DRT or DRC, iteration times should be required.
