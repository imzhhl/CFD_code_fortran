SUBROUTINE SENSITIVITY1(LOOP, Re, Gr, Pr)
...
!--- Inner Iteration!
      DO ITER = 1, INLOOP
         ...
         CALL   SUBTCNeuumanS1(LOOP)

         CALL   SUBUS1(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Re, Gr)

 	     CALL   SUBVS1(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,RE, Gr)

         CALL   SUBPCS1(ERROPC,OMIGPC)

         CALL   SUBTS1(ERROT,OMIGT,ISTATE,NSCHEMET,Pr,Re, Gr)
...
SUBROUTINE SUBTCNeuumanS1(loop)
...
! GCF GCFR: Gradient of the Cost Functional
      ...
      ! T(NI,J) = T(NIM,J)-CSD1(LOOP,J)*HDX(NIM)   	 !Note: the correct definition -k@t/@n = DeltaQ(?) it is independent of heat flux direction 
	     ! ENDDO
...
SUBROUTINE SUBUS1(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Re, Gr)
...
SUBROUTINE SUBVS1(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Re, Gr)
...
SUBROUTINE SUBTS1(ERROT,OMIGT,ISTATE,NSCHEMET,Pr,Re, Gr)
...
!---Save the loop numbers as output file name. !The maximum loop is no more than 10000.
	     ...
	     LLL =  LSAVE-1000*III-100*JJJ-10*KKK
	      ...
	      KC = CHAR(48+KKK)
	      LC = CHAR(48+LLL)
     FIField = 'SF1'//IC//JC//KC//LC//'.dat'	   !Plot the sensitivity convection field
