...
!--- Inner Iteration!
      DO ITER = 1, INLOOP
         ...
         CALL   SUBTCNeuumanS2(LOOP)

         CALL   SUBUS2(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Re, Gr)

 	     CALL   SUBVS2(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Re, Gr)

         CALL   SUBPCS2(ERROPC,OMIGPC)

         CALL   SUBTS2(ERROT,OMIGT,ISTATE,NSCHEMET,Pr,Re, Gr)
...
IF(LOOP==1.OR.MOD(LOOP,10)==0)THEN
CALL VISULIZATIONS2(LOOP)  
ENDIF  
...
SUBROUTINE SUBTCNeuumanS2(loop)
...
! GCF GCFR: Gradient of the Cost Functional
      ...
      ! T(1,J) = T(2,J)+CSD2(LOOP,J)*HDX(1)   	 !Note: the correct definition -k@t/@n = DeltaQ(?) it is independent of heat flux direction 
	     ! ENDDO
...
!RIGHT - the unknown Qu wall
                DO J = 2,NJM
                    U(NIM,J) = 0.0
                    V(NI, J) = 0.0
                    T(NI, J) = T(NIM,J) !- HDX(NIM) * CSD1(LOOP, J)
				ENDDO
				T(NI, 1) = T(NI,  2)
				T(NI,NJ) = T(NI,NJM)
...
SUBROUTINE SUBUS2(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Re, Gr)
   ...
   INCLUDE 'MESH.inc'
    ...
    COMMON/SVARIABLES2/U(NX,NY),V(NX,NY),T(NX,NY)
    COMMON/SPRESSURE2/ P(NX,NY)
    COMMON/SPREUV2/ BU(NX,NY),BV(NX,NY)
...
SUBROUTINE SUBVS2(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Re, Gr)
...
!   			 +                DX(I)*(T(I,J)*DY(J+1)+T(I,J+1)*DY(J))/2.0       ! For the form B
			 + (Gr / (Re * Re)) * DX(I)*(T(I,J)*DY(J+1)+T(I,J+1)*DY(J))/2.0       ! For the form C
...
SUBROUTINE SUBPCS2(ERROPC,OMIGPC)
...
SUBROUTINE SUBTS2(ERROT,OMIGT,ISTATE,NSCHEMET,Pr,Re, Gr)
...
SUBROUTINE VISULIZATIONS2(LSAVE)
 ...
 CHARACTER FIField*11, IC*1, JC*1, KC*1, LC*1
...
!---Save the loop numbers as output file name. !The maximum loop is no more than 10000.
	     ...
	     LLL =  LSAVE-1000*III-100*JJJ-10*KKK
	      ...
	      KC = CHAR(48+KKK)
	      LC = CHAR(48+LLL)
     FIField = 'SF2'//IC//JC//KC//LC//'.dat'	   !Plot the sensitivity convection field
