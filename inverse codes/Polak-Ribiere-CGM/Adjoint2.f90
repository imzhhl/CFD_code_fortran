SUBROUTINE ADJOINT2(LOOP, Re, Gr, Pr, FIQ2)
...
!--- Constant and Demo Parameters  
            OMIGU = 0.25;    OMIGV = 0.25;   OMIGPC = 0.90;    OMIGT = 0.80
...
!--- Inner Iteration!
      DO ITER = 1, INLOOP
         ...
         CALL   SUBUA2(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Re,Gr)

 	     CALL   SUBVA2(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Re,Gr)

         CALL  SUBPCA2(ERROPC,OMIGPC)

         CALL   SUBTA2(ERROT,OMIGT,NSCHEMET,Pr,Re,Gr,FIQ2)
...
SUBROUTINE SUBUA2(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Re,Gr)
...
SUBROUTINE SUBVA2(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Re,Gr)
...
SUBROUTINE SUBTA2(ERROT,OMIGT,NSCHEMET,Pr,Re,Gr, FIQ2)
...
!Save the loop numbers as output file name. !The maximum loop is no more than 10000.
	     ...
	     LLL =  LSAVE-1000*III-100*JJJ-10*KKK
	      ...
	      KC = CHAR(48+KKK)
	      LC = CHAR(48+LLL)
     FIField = 'AF2'//IC//JC//KC//LC//'.dat'	   !Plot the Adjoint field
