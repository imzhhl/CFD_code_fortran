SUBROUTINE ADJOINT1(LOOP, Re, Gr, Pr, FIQ)

   INCLUDE 'PARAMETER.inc'
   INCLUDE 'GRID.inc'
   INCLUDE 'MESH.inc'

    COMMON/AVARIABLES1/U(NX,NY),V(NX,NY),T(NX,NY)
    COMMON/APRESSURE1/ P(NX,NY)
...
!--- Constant and Demo Parameters  
            OMIGU = 0.20;    OMIGV = 0.20;   OMIGPC = 0.90;    OMIGT = 0.80
...
!--- Inner Iteration!
      DO ITER = 1, INLOOP
         ...
         CALL   SUBUA1(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Re, Gr)

 	     CALL   SUBVA1(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Re, Gr)

         CALL  SUBPCA1(ERROPC,OMIGPC)

         CALL   SUBTA1(ERROT,OMIGT,NSCHEMET,Pr,Re, Gr, FIQ)
...
IF(LOOP==1.OR.MOD(LOOP,1000)==0)THEN
CALL VISULIZATIONA1(LOOP)
ENDIF  
...
SUBROUTINE SUBUA1(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Re, Gr)
   ...
   COMMON/AVARIABLES1/U(NX,NY),V(NX,NY),T(NX,NY)
   COMMON/APRESSURE1/ P(NX,NY)
   COMMON/APREUV1/ BU(NX,NY),BV(NX,NY)
...
SUBROUTINE SUBVA1(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Re, Gr)
...
SUBROUTINE SUBPCA1(ERROPC,OMIGPC)
...
SUBROUTINE SUBTA1(ERROT,OMIGT,NSCHEMET,Pr,Re,Gr,FIQ)
   ...
   INCLUDE 'MESH.inc'
    ...
    COMMON/AVARIABLES1/U(NX,NY),V(NX,NY),T(NX,NY)
    COMMON/DVARIABLES1/UD(NX,NY),VD(NX,NY),TD(NX,NY)
...
!---Vertical Velocity term
        SOT4 = (Gr / (Re * Re)) * 0.5*(V(I,J-1) + V(I,J))*DX(I)*DY(J)
...
SUBROUTINE VISULIZATIONA1(LSAVE)
...
!Save the loop numbers as output file name. !The maximum loop is no more than 10000.
	     ...
	     LLL =  LSAVE-1000*III-100*JJJ-10*KKK
	      ...
	      KC = CHAR(48+KKK)
	      LC = CHAR(48+LLL)
     FIField = 'AF1'//IC//JC//KC//LC//'.dat'	   !Plot the Adjoint field
