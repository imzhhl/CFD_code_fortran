SUBROUTINE DIRECT(LOOP, Re, Gr， Pr, Q1, Q2)
...
!--- Inner Iteration!
      DO ITER = 1, INLOOP
         ...
         CALL   SUBUD(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Re, Gr)

 	     CALL   SUBVD(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Re, Gr)

         CALL  SUBPCD(ERROPC,OMIGPC)

         CALL   SUBTD(ERROT,OMIGT,NSCHEMET,Pr,Re, Gr)
...
!---Check the overall heat transfer rates
         ...
         !Calculate Global Heat Flux on the Top side
            ...
            ResHeat = TNuR  -  TNuL  +  TNuT  -  TNuB    !thermal flux Je - Jw + Jn - Js
             ...
             WRITE(*,110)  ITER, ERROU,ERROV,ERROPC,ERROT, ABS(ResFLOW), ABS(Resheat), 
             ENDIF
110         FORMAT(2X,I6,4X,6(E15.6E3,3X))
...
SUBROUTINE SUBUD(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Re, Gr)
...
SUBROUTINE SUBVD(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Re, Gr)
...
!   			 +                DX(I)*(T(I,J)*DY(J+1)+T(I,J+1)*DY(J))/2.0       ! For the form B
               +        (Gr/(Re**2.0))*DX(I)*(T(I,J)*DY(J+1)+T(I,J+1)*DY(J))/2.0       ! For the form B
...
SUBROUTINE SUBPCD(ERROPC,OMIGPC)
 ...
 DIMENSION AE(NX,NY),AW(NX,NY),AN(NX,NY),AS(NX,NY),SP(NX,NY),SOP(NX,NY),PC(NX,NY)
     ...
     IF(J.NE.NJM) V(I,J) = V(I,J)+BV(I,J)*(PC(I,J)-PC(I,J+1))
                  P(I,J) = P(I,J)+0.6*PC(I,J)   ! ??? why multiply 0.6?
          ENDDO
          ENDDO
...
SUBROUTINE SUBTD(ERROT,OMIGT,NSCHEMET,Pr,Re, Gr)
