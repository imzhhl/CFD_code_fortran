...
!    which is perpendicular with the dirction along the doundary 
 REAL Delta
! Control geometry augement for the inlet and exit openning
 INTEGER Lin, Luq
 COMMON/In_AND_Out/ Lin, Luq

! To simulate the measured temerature with rand error
 WRITE(*,*) "Enter the random error: Delta "
 READ(*,*) Delta
...
!NOTE[2]: The sensors should be located in the interior of fluid field.
              ...
              Imw1 = 100; Ime1 = 100
              Jms =  2; Jmn = NJM
              OPEN(31,FILE='T-measured100.dat',STATUS='UNKNOWN')
            DO Im = Imw1,Ime1 
            DO Jm = Jms-1,Jmn+1  
...
!            Delta = 0.05
!            Delta = 0.10
!         ConverCGM = 1.2*(Delta**2.0)/2.0  !Number of sensors 160.0
...
!Solve the direct problem for temperature TD
			 ...
			 CALL DIRECT(KCG, Re, Gr, Pr, Q1, Q2)
...
!
       !Sovle the adjoint problem backward in time for adjoint temperature TA
             ...
             CALL ADJOINT1(KCG, Re, Gr, Pr, FIQ)
...
! !define the Gradient of Cost Functional: GCF
	       ...
	       ELSE
             ...
             DO J = 2, NJM			          !--------------------------Depending on the unknown function (4)
               GAMAFZ1 = GAMAFZ1 + GCF1(KCG,J)*(GCF1(KCG,J)-GCF1(KCG-1,J))   ! Polak-Ribiere   CGM
               GAMAFM1 = GAMAFM1 + GCF1(KCG-1,J)*GCF1(KCG-1,J)			     ! Polak-Ribiere   CGM
...
! !Factor GAMA(KCG)/.or.BETA(KCG)/ (Factor for Search Direction)
	       ...
	       ELSE
             ...
             DO J = 2, NJM			          !--------------------------Depending on the unknown function (4)
             GAMAFZ2 = GAMAFZ2 + GCF2(KCG,J)*(GCF2(KCG,J)-GCF2(KCG-1,J))        !  Polak-Ribiere CGM
             GAMAFM2 = GAMAFM2 + GCF2(KCG-1,J)*GCF2(KCG-1,J)			        !  Polak-Ribiere CGM
...
!Solve the sensitivity problem for sensitivity temperature TS
        WRITE(*,*) ' Solve the Sensitivity1 Problem'
        CALL SENSITIVITY1(KCG, Re, Gr, Pr)
!Solve the sensitivity problem for sensitivity temperature TS
        WRITE(*,*) ' Solve the Sensitivity2 Problem'
        CALL SENSITIVITY2(KCG, Re, Gr, Pr)
...
! BELOWS ARE THE COEFFICIENTS NEEDED FOR THE CALCULATION OF THE STEP SIZE
		...
		ALPHA1FZ = V1*V2-V3*V4
		ALPHA1FM = V5*V2-V3**2
		ALPHA2FZ = V4*V5-V3*V1
		ALPHA1 = (V3*V1-V4*V5)/(V5*V2-V3**2)
		ALPHA2 = (V3*V4-V1*V2)/(V5*V2-V3**2)
		
		WRITE(*, 128) ALPHA1FZ,ALPHA1FM, ALPHA2FZ, ALPHA1, ALPHA2
128 FORMAT(5(E15.6E3,2X))
...
!
        DO J = 2 , NJM
		   ...
		   Q1(J) = Q1(J) + ALPHA1*CSD1(KCG,J)
		   !Q2(J) = Q2(J) + ALPHA2*CSD2(KCG,J)
