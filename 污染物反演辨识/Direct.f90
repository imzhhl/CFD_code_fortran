 SUBROUTINE DIRECT(LOOP, Re, Pr, Gr, ISX, ISY, SM, NIYDA, NIYDB)    ! three heat sources

   INCLUDE 'PARAMETER.inc'	 
   INCLUDE 'GRID.inc'
   INCLUDE 'MESH.inc'

    COMMON/DVARIABLES/U(NX,NY),V(NX,NY),T(NX,NY),C(NX,NY)
    COMMON/DPRESSURE/ P(NX,NY)
    COMMON/DCOEFENERGY/CoeffT(NX,NY)
    COMMON/DCOEFPOLLUT/CoeffC(NX,NY) 
...
!--- Inner Iteration!
      DO ITER = 1, INLOOP
		 ...
		 CALL   SUBTCNeuumanD(NIYDA,NIYDB)		   
        
		 CALL   SUBUD(ERROU,OMIGU,NSCHEMEUV,Re)  

 	     CALL   SUBVD(ERROV,OMIGV,NSCHEMEUV,Re,Gr,Br)  

         CALL   SUBPCD(ERROPC,OMIGPC)

         CALL   SUBTD(ERROT,OMIGT,NSCHEMET,Re,Pr)          

		 CALL   SUBCD(ERROC, OMIGC, NSCHEMEC, Re, Scc, SM, ISX, ISY)
...
SUBROUTINE SUBUD(ERROU,OMIGU,NSCHEMEUV, Re) 
...
SUBROUTINE SUBVD(ERROV,OMIGV,NSCHEMEUV,Re,Gr,Br)     !,JNNA,JNNB)
...
SUBROUTINE SUBTD(ERROT,OMIGT,NSCHEMET,Re,Pr) 
...
SUBROUTINE SUBCD(ERROC,OMIGC,NSCHEMEC,Re,Scc,SM,ISX,ISY) 
