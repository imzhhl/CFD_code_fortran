SUBROUTINE BRDIRECT(LOOP, Re, Pr, Gr, ISX, ISY, SM, NIYDA, NIYRDB)  ! three heat sources

   INCLUDE 'PARAMETER.inc'	 
   INCLUDE 'GRID.inc'
   INCLUDE 'MESH.inc'

    COMMON/BRDVARIABLES/U(NX,NY),V(NX,NY),T(NX,NY),C(NX,NY)
    COMMON/BRDPRESSURE/ P(NX,NY)
    COMMON/BRDCOEFENERGY/CoeffT(NX,NY)
    COMMON/BRDCOEFPOLLUT/CoeffC(NX,NY) 
...
!--- Inner Iteration!
      DO ITER = 1, INLOOP
		 ...
		 CALL   SUBTCNeuumanDRB(NIYDA,NIYRDB)		   

         CALL   SUBUDRB(ERROU,OMIGU,NSCHEMEUV,Re)  

 	     CALL   SUBVDRB(ERROV,OMIGV,NSCHEMEUV,Re,Gr,Br)  

         CALL   SUBPCDRB(ERROPC,OMIGPC)

         CALL   SUBTDRB(ERROT,OMIGT,NSCHEMET,Re,Pr)          

		 CALL   SUBCDRB(ERROC, OMIGC, NSCHEMEC, Re, Scc, SM, ISX, ISY)
...
!--* * * * * * * * * THE END OF THE PROGRAM MAIN     * * * * * * * * * *
 ...
 SUBROUTINE SUBTCNeuumanDRB(NIYDA,NIYRDB)
...
SUBROUTINE SUBUDRB(ERROU,OMIGU,NSCHEMEUV, Re) 
...
SUBROUTINE SUBVDRB(ERROV,OMIGV,NSCHEMEUV,Re,Gr,Br)     !,JNNA,JNNB)
...
SUBROUTINE SUBTDRB(ERROT,OMIGT,NSCHEMET,Re,Pr) 
...
SUBROUTINE SUBCDRB(ERROC,OMIGC,NSCHEMEC,Re,Scc,SM,ISX,ISY) 
...
!---Save the loop numbers as output file name. !The maximum loop is no more than 10000.
	     ...
	     LLL =  LSAVE-1000*III-100*JJJ-10*KKK
	      ...
	      KC = CHAR(48+KKK)
	      LC = CHAR(48+LLL)
     FIField = 'BRDF'//IC//JC//KC//LC	   !Plot the direct convection field
