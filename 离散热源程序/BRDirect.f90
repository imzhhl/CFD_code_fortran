SUBROUTINE BRDIRECT(LOOP, Ra, Pr, NIYDA, NIYRDB)
!SUBROUTINE BRDIRECT(LOOP, Ra, Pr, theta, NIYDA, NIYRDB, NIYDC)

   INCLUDE 'PARAMETER.inc'	 
   INCLUDE 'GRID.inc'
   INCLUDE 'MESH.inc'

    COMMON/BRDVARIABLES/U(NX,NY),V(NX,NY),T(NX,NY)
    COMMON/BRDPRESSURE/ P(NX,NY)

!    COMMON/BRDOBSTACLESPTCA/INWAR,INEAR,JNSAR !,JNNA
!    COMMON/BRDOBSTACLESPTCB/INWBR,INEBR,JNSBR !,JNNB
...
!--- Inner Iteration!
      DO ITER = 1, INLOOP
 		  ...
 		  CALL   SUBTCNeuumanDRB(NIYDA,NIYRDB)					! two heat sources
!         CALL   SUBTCNeuumanDRB(NIYDA,NIYRDB,NIYDC)				! three heat sources

         CALL   SUBUDRB(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Ra,theta)     !,JNNAR,JNNBR)

 	     CALL   SUBVDRB(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Ra,theta)     !,JNNAR,JNNBR)

         CALL  SUBPCDRB(ERROPC,OMIGPC)

         CALL   SUBTDRB(ERROT,OMIGT,NSCHEMET,Pr,Ra)             !,JNNAR,JNNBR)
...
!--* * * * * * * * * THE END OF THE PROGRAM MAIN     * * * * * * * * * *
 ...
 SUBROUTINE SUBTCNeuumanDRB(NIYDA,NIYRDB)								! two heat sources 						 
!SUBROUTINE SUBTCNeuumanDRB(NIYDA,NIYRDB,NIYDC)							! three heat sources
...
!LEFT - the unknown Qu2 wall
						  ...
						  T(1,J) = T(2,J) + HDX(1)
							...
							DO J = NIYDA+10,NIYRDB-1
						  T(1,J) = T(2,J)
					         ENDDO

							DO J = NIYRDB,NIYRDB+9
						  T(1,J) = T(2,J) + HDX(1)
					         ENDDO							

							DO J = NIYRDB+10,NJM    !NIYDC-1
						  T(1,J) = T(2,J)
					         ENDDO																					
...
SUBROUTINE SUBUDRB(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Ra,theta)  !,JNNAR,JNNBR)
   ...
   INCLUDE 'MESH.inc'
    ...
    COMMON/BRDVARIABLES/U(NX,NY),V(NX,NY),T(NX,NY)
    COMMON/BRDPRESSURE/ P(NX,NY)
    COMMON/BRPREUV/ BU(NX,NY),BV(NX,NY)
...
SUBROUTINE SUBVDRB(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Ra,theta)      !,JNNAR,JNNBR)
...
SUBROUTINE SUBPCDRB(ERROPC,OMIGPC)
...
SUBROUTINE SUBTDRB(ERROT,OMIGT,NSCHEMET,Pr,Ra)  !,JNNAR,JNNBR)
...
SUBROUTINE VISULIZATIONDRB(LSAVE)
