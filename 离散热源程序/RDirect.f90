SUBROUTINE RDIRECT(LOOP, Ra, Pr, NIY2)

   INCLUDE 'PARAMETER.inc'	 
   INCLUDE 'GRID.inc'
   INCLUDE 'MESH.inc'

    COMMON/RDVARIABLES/U(NX,NY),V(NX,NY),T(NX,NY)
    COMMON/RDPRESSURE/ P(NX,NY)
...
!--- Partitions on the bottom side
		!	 INWAR = 46;   INEAR = 47;    JNSAR = 2;      JNNAR = NIY2
		!	 INWBR = 86;   INEBR = 87;    JNSBR = 2;      JNNBR = 72 - NIY2
...
WRITE(*,*) ' (3.1)The NIY2 could be', JNNAR
WRITE(*,*) ' (3.2)The IRDNIY could be', JNSBR
...
!--- Inner Iteration!
      DO ITER = 1, INLOOP
         ...
         CALL   SUBUDR(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Ra,JNNAR,JNNBR)

 	     CALL   SUBVDR(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Ra,JNNAR,JNNBR)

         CALL  SUBPCDR(ERROPC,OMIGPC)
         ...
         CALL   SUBTDR(ERROT,OMIGT,NSCHEMET,Pr,Ra,JNNAR,JNNBR)
...
SUBROUTINE SUBUDR(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Ra,JNNAR,JNNBR)
   ...
   INCLUDE 'MESH.inc'
    ...
    COMMON/RDVARIABLES/U(NX,NY),V(NX,NY),T(NX,NY)
    COMMON/RDPRESSURE/ P(NX,NY)
    COMMON/RPREUV/ BU(NX,NY),BV(NX,NY)
 ...
 DIMENSION AU(NX,NY)
       ...
       COEFU = SQRT(Pr/Ra)    ! Form B
         ...
         NIM = NI-2;   NJM = NJ-1
		  ...
		  AU = 0.0
!*****************DA XI SHU FA******************************************************!                  
				  DO I = INWAR-1,INEAR		   ![OPEN] series of computational grids
	              DO J = JNSAR,  JNNAR
               AU(I,J) = 100000000000.0
                  ENDDO
				  ENDDO
				  
				  DO I = INWBR-1,INEBR		   ![OPEN] series of computational grids
               	  DO J = JNSBR,  JNNBR
			   AU(I,J) = 100000000000.0
                  ENDDO
				  ENDDO
...
!---Relaxation through inertia 
    ...
    SOU(I,J) = DY(J)*(P(I,J)-P(I+1,J))+STIME+SAD 
     ...
     SU(I,J) = AE(I,J)+AW(I,J)+AN(I,J)+AS(I,J)+CTIME + AU(I,J)
...
SUBROUTINE SUBVDR(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Ra,JNNAR,JNNBR)
...
!      COEFV = Pr              ! Form A
       COEFV = SQRT(Pr/Ra)     ! Form B
		 ...
		 AV = 0.0
!*****************DA XI SHU FA******************************************!				  
				  DO I = INWAR,  INEAR		   ![OPEN] series of computational grids
	              DO J = JNSAR-1,JNNAR
               AV(I,J) = 100000000000.0
                  ENDDO
				  ENDDO
				  
				  DO I = INWBR,  INEBR		   ![OPEN] series of computational grids
               	  DO J = JNSBR-1,JNNBR
			   AV(I,J) = 100000000000.0
                  ENDDO
				  ENDDO
...
SUBROUTINE SUBPCDR(ERROPC,OMIGPC)
...
SUBROUTINE SUBTDR(ERROT,OMIGT,NSCHEMET,Pr,Ra,JNNAR,JNNBR)
...
DO J = 2,NJM
		...
		VEFe = (FX(I)/COEFT(I,J)+(1.0-FX(I))/COEFT(I+1,J))**(-1)     ! Harmonic Mean of P and E                
          De = VEFe*DY(J)/HDX(I)
          Fe = U(I,J)*DY(J)
     AE(I,J) = De * SCHEME(Fe/De,NSCHEMET)+AMAX1(-Fe,0.0)

        VEFw = (FX(I-1)/COEFT(I-1,J)+(1.0-FX(I-1))/COEFT(I,J))**(-1)	! Harmonic Mean of W and P
          Dw = VEFw*DY(J)/HDX(I-1)
          Fw = U(I-1,J)*DY(J)
     AW(I,J) = Dw * SCHEME(Fw/Dw,NSCHEMET)+AMAX1( Fw,0.0)
	
	    VEFn = (FY(J)/COEFT(I,J)+(1.0-FY(J))/COEFT(I,J+1))**(-1) 
          Dn = VEFn*DX(I)/HDY(J)
          Fn = V(I,J)*DX(I)
     AN(I,J) = Dn * SCHEME(Fn/Dn,NSCHEMET)+AMAX1(-Fn,0.0)
	  
	    VEFs = (FY(J-1)/COEFT(I,J-1)+(1.0-FY(J-1))/COEFT(I,J))**(-1)
          Ds = VEFs*DX(I)/HDY(J-1)
          Fs = V(I,J-1)*DX(I)
...
SUBROUTINE VISULIZATIONDR(LSAVE)
