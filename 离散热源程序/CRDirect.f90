 SUBROUTINE CRDIRECT(LOOP, Ra, Pr, theta, NIYDA, NIYDB, NIYRDC)

   INCLUDE 'PARAMETER.inc'	 
   INCLUDE 'GRID.inc'
   INCLUDE 'MESH.inc'

    COMMON/CRDVARIABLES/U(NX,NY),V(NX,NY),T(NX,NY)
    COMMON/CRDPRESSURE/ P(NX,NY)
...
!--- Inner Iteration!
      DO ITER = 1, INLOOP
         ...
         CALL   SUBTCNeuumanDRC(NIYDA,NIYDB,NIYRDC)

         CALL   SUBUDRC(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Ra,theta)     !,JNNAR,JNNBR)

 	     CALL   SUBVDRC(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Ra,theta)     ! ,JNNAR,JNNBR)

         CALL  SUBPCDRC(ERROPC,OMIGPC)

         CALL   SUBTDRC(ERROT,OMIGT,NSCHEMET,Pr,Ra)              ! ,JNNAR,JNNBR)
...
SUBROUTINE SUBTCNeuumanDRC(NIYDA,NIYDB,NIYRDC)
...
!LEFT - the unknown Qu2 wall
						  ...
						  T(1,J) = T(2,J) + HDX(1)
							...
							DO J = NIYDB+10,NIYRDC-1
						  T(1,J) = T(2,J)
					         ENDDO																					

							DO J = NIYRDC,NIYRDC+10
						  T(1,J) = T(2,J) + HDX(1)
					         ENDDO							
...
SUBROUTINE SUBUDRC(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Ra,theta)  !,JNNAR,JNNBR)
   ...
   INCLUDE 'MESH.inc'
    ...
    COMMON/CRDVARIABLES/U(NX,NY),V(NX,NY),T(NX,NY)
    COMMON/CRDPRESSURE/ P(NX,NY)
    COMMON/CRPREUV/ BU(NX,NY),BV(NX,NY)
...
SUBROUTINE SUBVDRC(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Ra,theta)      !,JNNAR,JNNBR)
...
SUBROUTINE SUBPCDRC(ERROPC,OMIGPC)
...
SUBROUTINE SUBTDRC(ERROT,OMIGT,NSCHEMET,Pr,Ra)  !,JNNAR,JNNBR)
...
SUBROUTINE VISULIZATIONDRC(LSAVE)
