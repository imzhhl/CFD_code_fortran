!SUBROUTINE DIRECT(LOOP, Ra, Pr, NIYDA)				   ! one heat sources
SUBROUTINE DIRECT(LOOP, Ra, Pr, NIYDA, NIYDB)		   ! two heat sources
!SUBROUTINE DIRECT(LOOP, Ra, Pr, theta, NIYDA, NIYDB, NIYDC)  ! three heat sources
...
!    COMMON/DOBSTACLESPTCA/INWA,INEA,JNSA 
!    COMMON/DOBSTACLESPTCB/INWB,INEB,JNSB 
...
!--- Partitions on the bottom and top side	C = 0.9(11),0.7(31),0.5(51)		  
!			 INWA = 31;   INEA = 32;    JNSA = 2;       JNNA = NIY1
!			 INWB = 71;   INEB = 72;    JNSB = NIY1+11; JNNB = NJM
...
!--- Inner Iteration!
      DO ITER = 1, INLOOP
		...
		!CALL   SUBTCNeuumanD(NIYDA)					   ! one heat source
		 CALL   SUBTCNeuumanD(NIYDA,NIYDB)				   ! two heat sources
		!CALL   SUBTCNeuumanD(NIYDA,NIYDB,NIYDC)		   ! three heat sources

         CALL   SUBUD(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Ra,theta)  !,JNNA,JNNB)

 	     CALL   SUBVD(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Ra,theta)  !,JNNA,JNNB)

         CALL  SUBPCD(ERROPC,OMIGPC)

         CALL   SUBTD(ERROT,OMIGT,NSCHEMET,Pr,Ra)          !,JNNA,JNNB)
...
!---Check the overall heat transfer rates
         ...
         !Calculate Global Heat Flux on the Top side
            ...
            !ResHeat = TNuR  -  TNuL  +  TNuT  -  TNuB    !thermal flux Je - Jw + Jn - Js
             ...
             ENDIF
110         FORMAT(2X,I6,4X,5(E8.3E1,3X))						
...
!SUBROUTINE SUBTCNeuumanD(NIYDA)														 
 SUBROUTINE SUBTCNeuumanD(NIYDA,NIYDB)
!SUBROUTINE SUBTCNeuumanD(NIYDA,NIYDB,NIYDC)
...
SUBROUTINE SUBUD(ERROU,OMIGU,ISTATE,NSCHEMEUV,Pr,Ra,theta)   !,JNNA,JNNB)
...
!    COMMON/DOBSTACLESPTCA/INWA,INEA,JNSA !,JNNA
!    COMMON/DOBSTACLESPTCB/INWB,INEB,JNSB !,JNNB
...
SUBROUTINE SUBVD(ERROV,OMIGV,ISTATE,NSCHEMEUV,Pr,Ra,theta)     !,JNNA,JNNB)
...
SUBROUTINE SUBTD(ERROT,OMIGT,NSCHEMET,Pr,Ra)         !,JNNA,JNNB)
...
!---Save the loop numbers as output file name. !The maximum loop is no more than 10000.
	     ...
	     LLL =  LSAVE-1000*III-100*JJJ-10*KKK
	      ...
	      KC = CHAR(48+KKK)
	      LC = CHAR(48+LLL)
     FIField = 'DF'//IC//JC//KC//LC	   !Plot the direct convection field
